...
# Include federal legislation so CI restore cannot wipe a short rolling window
# over a previously merged year-long corpus (quiet bills stay searchable).
PIPELINE_UPLOAD_GLOBS = [
    "data/normalized/*.json",
    "data/openstates/*/bills*.json",
    "data/openstates/*/votes.json",
    "src/output/legislation.json",
    "src/output/congress_votes.json",
    "data/congress/*.json",
]
...
def write_data_config() -> Path:
    ...
    api_base = (
        os.environ.get("POLICYWATCH_API_BASE")
        or "https://policywatch-api.wesley-a-emberlin.workers.dev"
    ).strip().rstrip("/")
    ...
    contents = (
        ...
        f"window.POLICYWATCH_API_BASE = {api_base!r};\n"
    )
    out.write_text(contents, encoding="utf-8")
    print(f"Wrote {out} with base={base or '(empty — same-origin)'} api={api_base}")
    return out
